<template>
  <div>
    <div class="row">
      <div class="col-md-12">
        <h4>Currencies</h4>
        <h5>List all currencies for rooms.</h5>
        <br>
      </div>
    </div>
    <div class="row">
      <div class="col-md-6 form-inline">
        <router-link to="/currencies/add" class="btn btn-success btn-sm">Create</router-link>
      </div>
    ...
    <div class="row">
      <div class="col">
        <table class="table table-sm table-hover">
          ...
          <tbody v-if="!!records">
            <tr v-for="(rec, index) in records" v-bind:key="index">
              ...
              <td>
                ...
                <small><strong>Symbol: </strong>{{ rec.symbol }} as {{ rec.symbol_location}} </small>
              </td>
              <td><small>{{rec.code}}</small></td>
              <td><small>{{rec.created_at}}</small></td>
              <td><small>{{rec.updated_at}}</small></td>
              <td>
                <div class="btn-group" role="group">
                  <router-link :to="'/currencies/update/'+rec.id" class="btn btn-primary btn-sm">Edit</router-link>
                  <button
                    type="button"
          ...
          <tbody v-else>
            <tr>
              <td colspan="4">Sorry, data currencies is empty.</td>
            </tr>
          </tbody>
        </table>
